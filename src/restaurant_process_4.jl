...
mutable struct RestaurantProcess4 <: AbstractGuide
    delta::Vector{Float64}
    alpha::Vector{Float64}
    rho::Vector{Float64}
end

RestaurantProcess4(; delta::Float64, alpha::Float64, rho::Float64) = RestaurantProcess4([delta],[alpha],[rho])

Flux.@layer RestaurantProcess4

function clamp!(guide::RestaurantProcess4)
...
function corrMatrixFun(guide::RestaurantProcess4, observation::Vector{Float64})
...
function sample(
    ...
    u = Random.rand(rng)
    if u < 0.2
        partition_local = [collect(1:d)]
        logLikelihood = log(0.2)
    elseif u < 0.4
        partition_local = [[x] for x in 1:d] 
        logLikelihood = log(0.2)
    else

        logLikelihood = log(0.6)
